...
- ALU completion buffer、带缓冲的 MUL/DIV producer、单请求 LSU 和四路 round-robin CDB 仲裁；
- 统一 PRF writeback、ROB completion、依赖唤醒和顺序 Commit；
- Branch/JAL/JALR actual-next-PC 解析及退休时错误预测恢复；
- instruction/data access fault、地址未对齐、illegal instruction、ECALL 和 EBREAK 的精确异常 metadata；
- ROB-Head-only Load/Store、外部 data-memory port 和访存 Commit record；
- trap 退休后的 sticky halt。

以下能力尚未接入 OoO core：

- OoO core 的 L1 instruction/data cache 接入，以及允许提前 Load 的 LSQ；
...
RV32M 和单请求 LSU 均已连接到 backend/core，并有独立单元测试和 core-level 定向程序。
现有 L1 cache 仍只接入 pipeline；OoO 集成测试的 instruction/data port 都直连 memory model。
...
每周期最多发射一条指令，所以四个执行路径共享 PRF 的两个数据读端口。`fu_kind` 选择
ALU/branch、MUL、DIV 或 MEMORY；各自的 `issue_ready` 回到 `fu_ready`，一个繁忙的功能单元不会
阻止其他已就绪类型发射。Issue Stage 始终用选中 uop 的 source tag 驱动 PRF 地址，
MUL/DIV/LSU wrapper 直接读取同一组 `prf_rdata1/2`。Memory uop 还必须等其 ROB tag 等于
Head tag 才能离开 Issue Queue；年轻 ALU/MUL/DIV 仍可执行，但不能越过它退休。
...
- 完成时产生的访存地址、byte mask、原始读数据或写数据；
- valid、completed 和 generation 状态。

四个 completion producer 使用相同的 ready/valid 协议与 `completion_payload_t`：
...
| LSU | 一项请求和 held completion | 前一访存的 response 与 completion 均处理完毕 |
...
LSU 另外把运行时 trap 和 memory Commit record 连同 ROB tag 一起送入 CDB；Store 不写 PRF，
但必须等 memory response 后才能标记 ROB 完成。LSU 的 valid/payload 只由保存的状态产生，
不组合依赖 CDB ready，避免 ready/valid 环路。
...
ROB 到达 Head 后才对外报告 memory Commit record。运行时 Load/Store fault 不产生寄存器或
memory Commit 副作用；trap cause 随该 entry 一起退休。
...
- 取消 LSU 未接受的请求；若 memory 已接受请求，则先丢弃迟到 response，再接收新 uop；
- 抑制共享 CDB 广播并复位仲裁优先级；
...
Load/Store 未对齐由 LSU 在发 memory request 前发现；memory response error 则生成 access
fault。这些运行时 trap 随 memory completion 写入对应 ROB entry，Load 不写 PRF。
...
`tb/core/rv32_ooo_memory_tb.sv` 检查 SW/LW、SB/SH 与有符号/无符号 Load 的逐条 Commit，
DIV 前后的 ROB-Head-only 请求、错误路径 Store、地址未对齐与 access fault，以及 request 和
response 分别受阻时的保持行为。它使用 `rv32_simple_memory`，不是 L1 cache 测试。
...
单独运行 OoO execution 和 memory 测试：

```bash
make CAD_ENV=/path/to/env.sh test-ooo-lsu
make CAD_ENV=/path/to/env.sh test-ooo-rv32m test-ooo-core test-ooo-memory
```

`test-ooo-lsu` 验证单请求 LSU 的格式、异常、backpressure 和 Flush；
`test-ooo-memory` 验证 Decode 到 architectural Commit 的完整访存路径。
`compile-ooo-rv32m` 和 `compile-ooo-core` 只构建仿真程序，对应的 `test-*` target 才会执行。
这些测试都已包含在默认 `make test` 中。工具已在 PATH 中时可以省略 `CAD_ENV`。

## 8. Next Integration Stages

1. O6 后续：把现有 L1 cache 接到 OoO core，保留相同的外部 Commit 语义；
