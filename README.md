...
flowchart LR
    ...
    class PC,FETCH,L1I,DECODE,ALU,MULDIV,LSU,COMMIT,ARF implemented
    class L1D,L2,RENAME,RAT,ROB,RS,COMPLETE planned
    class MEM external
```

Green nodes identify implemented capabilities, including the pipeline-integrated L1 I-cache. Gray nodes are planned cache or OoO structures, while blue identifies the external memory environment. Green does not imply that a block has already been integrated into the future OoO core.
...
- implemented I-cache hit, miss, refill, replacement, backpressure, and error tests, with D-cache writeback and byte-mask tests planned.
...
The RV32IM multicycle reference core, five-stage in-order pipeline, and blocking direct-mapped L1 instruction cache are implemented and covered by self-checking regressions. The L1 data cache, L2 cache, external-model differential verification, and out-of-order core remain planned. Implemented components include:
...
绿色节点表示已经实现的能力，包括与 pipeline 集成的 L1 I-cache；灰色节点表示计划中的 cache 或 OoO 结构；蓝色节点表示外部 memory 环境。绿色不代表该模块已经完成未来 OoO core 中的集成。
...
- 已实现的 I-cache hit、miss、refill、replacement、backpressure 和 error 测试，以及计划中的 D-cache writeback 和 byte-mask 测试。
...
RV32IM 多周期 reference core、五级顺序流水线和 blocking direct-mapped L1 instruction cache 已经实现，并具有 self-checking regression。L1 data cache、L2 cache、外部模型差分验证和乱序核仍属于后续计划。当前已实现内容包括：
