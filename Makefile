...
OOO_MULTIPLIER_TEST := $(BUILD_DIR)/rv32_ooo_multiplier_tb
OOO_MULTIPLIER_SRCS := \
	rtl/pkg/rv32_pkg.sv \
	rtl/pkg/rv32_core_pkg.sv \
	rtl/pkg/rv32_ooo_pkg.sv \
...
OOO_LSU_TEST := $(BUILD_DIR)/rv32_ooo_lsu_tb
OOO_LSU_SRCS := \
	rtl/pkg/rv32_pkg.sv \
	rtl/pkg/rv32_core_pkg.sv \
	rtl/pkg/rv32_ooo_pkg.sv \
	rtl/backend/rv32_lsu.sv \
	rtl/backend/rv32_ooo_lsu.sv \
	tb/unit/rv32_ooo_lsu_tb.sv
...
OOO_MEMORY_TEST := $(BUILD_DIR)/rv32_ooo_memory_tb
# Keep both core regressions on the same RTL source list.
OOO_MEMORY_SRCS := $(filter-out tb/core/rv32_ooo_core_tb.sv,$(OOO_CORE_SRCS)) \
	tb/model/rv32_simple_memory.sv \
	tb/core/rv32_ooo_memory_tb.sv
...
.PHONY: test test-alu test-regfile test-fetch-queue test-ooo-frontend test-rename-dispatch test-ooo-execute test-cdb-arbiter test-ooo-multiplier test-phys-regfile test-rename-map test-free-list test-issue-queue test-rob test-rob-storage test-rob-completion test-rob-retirement test-imm-gen test-decoder test-branch-unit test-hazard-unit test-forwarding-unit test-lsu \
...
.PHONY: compile-cdb-arbiter compile-ooo-multiplier compile-ooo-divider test-ooo-divider compile-ooo-lsu test-ooo-lsu compile-ooo-rv32m test-ooo-rv32m
...
.PHONY: compile-ooo-integer test-ooo-integer compile-ooo-core test-ooo-core test-ooo-memory

test: test-alu test-regfile test-fetch-queue test-ooo-frontend test-rename-dispatch test-ooo-execute test-cdb-arbiter test-ooo-multiplier test-ooo-divider test-ooo-lsu test-ooo-rv32m test-ooo-integer test-ooo-core test-ooo-memory test-phys-regfile test-rename-map test-free-list test-issue-queue test-rob test-rob-storage test-rob-completion test-rob-retirement test-imm-gen test-decoder test-branch-unit test-lsu test-multiplier test-divider \
...
# Compile and run the out-of-order frontend Fetch Queue regression.
test-fetch-queue: $(FETCH_QUEUE_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(FETCH_QUEUE_TEST): $(FETCH_QUEUE_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
...
# Compile and run the atomic Rename/Dispatch regression.
test-rename-dispatch: $(RENAME_DISPATCH_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(RENAME_DISPATCH_TEST): $(RENAME_DISPATCH_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
		$(IVERILOG) -g2012 -Wall -s rv32_rename_dispatch_tb -o $@ $(RENAME_DISPATCH_SRCS)'

# Compile and run the integer Issue Stage and registered Completion Buffer regressions.
test-ooo-execute: $(OOO_ISSUE_STAGE_TEST) $(OOO_COMPLETION_BUFFER_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $(OOO_ISSUE_STAGE_TEST)'
	bash -c '$(ENV_SETUP) $(VVP) $(OOO_COMPLETION_BUFFER_TEST)'

$(OOO_ISSUE_STAGE_TEST): $(OOO_ISSUE_STAGE_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
		$(IVERILOG) -g2012 -Wall -s rv32_issue_stage_tb -o $@ $(OOO_ISSUE_STAGE_SRCS)'

$(OOO_COMPLETION_BUFFER_TEST): $(OOO_COMPLETION_BUFFER_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
...
# Compile or run the buffered OoO multiplier completion-path regression.
compile-ooo-multiplier: $(OOO_MULTIPLIER_TEST)

test-ooo-multiplier: $(OOO_MULTIPLIER_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(OOO_MULTIPLIER_TEST): $(OOO_MULTIPLIER_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
...
# Compile or run the OoO LSU request, completion, fault, and recovery regression.
compile-ooo-lsu: $(OOO_LSU_TEST)

test-ooo-lsu: $(OOO_LSU_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(OOO_LSU_TEST): $(OOO_LSU_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
		$(IVERILOG) -g2012 -Wall -s rv32_ooo_lsu_tb -o $@ $(OOO_LSU_SRCS)'
...
# Exercise the ordered data port and architectural memory Commit path.
test-ooo-memory: $(OOO_MEMORY_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(OOO_MEMORY_TEST): $(OOO_MEMORY_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
		$(IVERILOG) -g2012 -Wall -s rv32_ooo_memory_tb -o $@ $(OOO_MEMORY_SRCS)'
...
# Compile and run the physical-register free-list regression.
test-free-list: $(FREE_LIST_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(FREE_LIST_TEST): $(FREE_LIST_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
...
# Compile and run the ROB in-order retirement and backpressure regression.
test-rob-retirement: $(ROB_RETIREMENT_TEST)
	bash -c '$(ENV_SETUP) $(VVP) $<'

$(ROB_RETIREMENT_TEST): $(ROB_RETIREMENT_SRCS)
	mkdir -p $(BUILD_DIR)
	bash -c '$(ENV_SETUP) \
		$(IVERILOG) -g2012 -Wall -s rv32_rob_retirement_tb -o $@ $(ROB_RETIREMENT_SRCS)'
...
clean:
	rm -f $(FETCH_QUEUE_TEST) $(OOO_FRONTEND_TEST) $(RENAME_DISPATCH_TEST) $(OOO_ISSUE_STAGE_TEST) $(OOO_COMPLETION_BUFFER_TEST) $(CDB_ARBITER_TEST) $(OOO_MULTIPLIER_TEST) $(OOO_DIVIDER_TEST) $(OOO_LSU_TEST) $(OOO_RV32M_TEST) $(OOO_INTEGER_TEST) $(OOO_CORE_TEST) $(OOO_MEMORY_TEST) $(ISSUE_QUEUE_TEST)
